...
export default function ProfileCard({
  ...
  return (
    <div className="md:col-span-7 rounded-3xl bg-white/40 dark:bg-slate-800/50 backdrop-blur-md border border-white/40 dark:border-white/10 shadow-xl p-5 sm:p-6 md:p-8 flex flex-col justify-between transition-all duration-700 hover:scale-[1.01] group relative overflow-hidden h-full min-h-[220px] md:min-h-[280px]">
      ...
      <Link
        href="/about"
        className="flex items-start justify-between relative z-10 rounded-2xl focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500/50"
        aria-label="打开关于页面"
      >
      ...
      <div className="flex flex-col md:flex-row items-center md:items-end justify-between mt-6 md:mt-8 gap-5 md:gap-6 relative z-20">
        ...
        <div className="flex gap-2 md:gap-3 flex-wrap justify-center md:justify-end w-full md:w-auto">
          ...
          <SocialBtn
            type="email"
            onClick={() =>
              copyToClipboard(siteConfig.social?.email || '', '邮箱')
            }
          />
          ...
          <SocialBtn
            type="qq"
            onClick={() =>
              copyToClipboard(siteConfig.social?.qq || '', 'QQ号')
            }
          />
          ...
          <SocialBtn
            type="wechat"
            onClick={() =>
              copyToClipboard(siteConfig.social?.wechat || '', '微信号')
            }
          />
        </div>
      </div>
...
function StatLink({
  ...
  return (
    <Link
      href={href}
      className="text-center group/stat px-2 py-1 rounded-xl cursor-pointer focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500/50"
      aria-label={`打开${label}`}
      title={`打开${label}`}
    >
      <div
        className={`text-xl md:text-2xl font-black ${color} transition-transform duration-200 group-hover/stat:scale-110`}
      >
        {count}
      </div>

      <div className="text-[9px] md:text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mt-0.5 group-hover/stat:text-slate-800 dark:group-hover/stat:text-white transition-colors">
        {label}
      </div>
    </Link>
  );
