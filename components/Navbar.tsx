...
export default function Navbar() {
  ...
  // PC 滚动导航性能优化：
  // 不再把 lastScrollY 放进 state，也不会每次滚动重绑监听器。
  ...
  const handlePan = (
    ...
    let deltaAngle =
      (currAngle - prevAngle) * (180 / Math.PI);

    if (deltaAngle > 180) deltaAngle -= 360;
    if (deltaAngle < -180) deltaAngle += 360;

    rawRotation.set(
      rawRotation.get() + deltaAngle
    );
  };
  ...
  return (
    <>
      <header
        className={`hidden md:block w-full fixed top-0 left-0 right-0 z-50 transition-transform duration-500 border-b ${
          showNav
            ? 'translate-y-0'
            : '-translate-y-full'
